...
function uploadCss( cssData ) {
	...
	.then( function() {
		console.log( 'upload of css successful' );
	} )
	.catch( function( err ) {
...
function uploadPost( postData ) {
	...
	.then( function() {
		console.log( 'upload of post ID ' + postData.ID + ' successful:', postData.title );
	} )
	.catch( function( err ) {
...
function uploadChangedFile( filePath ) {
	if ( filePath === Styles.getFilename() ) {
		return Styles.readStyles()
		.then( function() {
			return uploadCss( { 'css': Styles.getStyles(), 'preprocessor': Styles.getPreprocessor() } );
		} );
	}
	var jsonFilename = path.basename( filePath, path.extname( filePath ) ) + '.json';
	return readAndUploadPost( jsonFilename );
}

module.exports = {
	uploadSite: uploadSite,
	uploadStylesForSite: uploadStylesForSite,
	uploadPostsForSite: uploadPostsForSite,
